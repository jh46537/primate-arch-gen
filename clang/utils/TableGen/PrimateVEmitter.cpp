...
//};
//
//// This class is compact representation of a valid and invalid PRVType.
//class PRVType {
...
//
//public:
//  PRVType() : PRVType(BasicType(), 0, StringRef()) {}
//  PRVType(BasicType BT, int Log2LMUL, StringRef prototype);
//
//  // Return the string representation of a type, which is an encoded string for
//  // passing to the BUILTIN() macro in Builtins.def.
//  const std::string &getBuiltinStr() const { return BuiltinStr; }
//
//  // Return the clang buitlin type for PRV vector type which are used in the
//  // primate_vector.h header file.
...
//
//private:
//  // Verify PRV vector type and set Valid.
...
//  void initBuiltinStr();
//  // Compute and record a builtin PRV vector type string.
...
//};
//
//using PRVTypePtr = PRVType *;
//using PRVTypes = std::vector<PRVTypePtr>;
//
//enum PrimateExtension : uint8_t {
//  Basic = 0,
//  F = 1 << 1,
//  D = 1 << 2,
//  Zfh = 1 << 3,
...
//};
//
//// TODO refactor PRVIntrinsic class design after support all intrinsic
...
//  PRVTypePtr OutputType; // Builtin output type
//  PRVTypes InputTypes;   // Builtin input types
//  // The types we use to obtain the specific LLVM intrinsic. They are index of
//  // InputTypes. -1 means the return type.
//  std::vector<int64_t> IntrinsicTypes;
//  uint8_t PrimateExtensions = 0;
//  unsigned NF = 1;
//
//public:
//  PRVIntrinsic(StringRef Name, StringRef Suffix, StringRef MangledName,
...
//               StringRef ManualCodegen, const PRVTypes &Types,
//               const std::vector<int64_t> &IntrinsicTypes,
//               StringRef RequiredExtension, unsigned NF);
//  ~PRVIntrinsic() = default;
//
...
//  uint8_t getPrimateExtensions() const { return PrimateExtensions; }
//  unsigned getNF() const { return NF; }
//
//  // Return the type string for a BUILTIN() macro in Builtins.def.
//  std::string getBuiltinTypeStr() const;
//
//  // Emit the code block for switch body in EmitPrimateBuiltinExpr, it should
//  // init the PRVIntrinsic ID and IntrinsicTypes.
...
//  void createPRVIntrinsics(std::vector<std::unique_ptr<PRVIntrinsic>> &Out);
//  /// Compute output and input types by applying different config (basic type
//  /// and LMUL with type transformers). It also record result of type in legal
//  /// or illegal set to avoid compute the  same config again. The result maybe
//  /// have illegal PRVType.
//  Optional<PRVTypes> computeTypes(BasicType BT, int Log2LMUL, unsigned NF,
//                                  ArrayRef<std::string> PrototypeSeq);
//  Optional<PRVTypePtr> computeType(BasicType BT, int Log2LMUL, StringRef Proto);
//
//  /// Emit Acrh predecessor definitions and body, assume the element of Defs are
//  /// sorted by extension.
//  void emitArchMacroAndBody(
//      std::vector<std::unique_ptr<PRVIntrinsic>> &Defs, raw_ostream &o,
//      std::function<void(raw_ostream &, const PRVIntrinsic &)>);
//
...
//}
//
//PRVType::PRVType(BasicType BT, int Log2LMUL, StringRef prototype)
...
//}
//
//void PRVType::initBuiltinStr() {
//  assert(isValid() && "PRVType is invalid");
//  switch (ScalarType) {
...
//}
//
//void PRVType::initClangBuiltinStr() {
//  assert(isValid() && "PRVType is invalid");
//  assert(isVector() && "Handle Vector type only");
//
//  ClangBuiltinStr = "__prv_";
//  switch (ScalarType) {
//  case ScalarTypeKind::Boolean:
...
//}
//
//void PRVType::applyModifier(StringRef Transformer) {
...
//
////===----------------------------------------------------------------------===//
//// PRVIntrinsic implementation
////===----------------------------------------------------------------------===//
//PRVIntrinsic::PRVIntrinsic(StringRef NewName, StringRef Suffix,
...
//                           bool HasNoMaskedOverloaded, bool HasAutoDef,
//                           StringRef ManualCodegen, const PRVTypes &OutInTypes,
...
//    Name += "_m";
//  }
//  // Init Primate extensions
//  for (const auto &T : OutInTypes) {
//    if (T->isFloatVector(16) || T->isFloat(16))
//      PrimateExtensions |= PrimateExtension::Zfh;
//    else if (T->isFloatVector(32) || T->isFloat(32))
//      PrimateExtensions |= PrimateExtension::F;
//    else if (T->isFloatVector(64) || T->isFloat(64))
//      PrimateExtensions |= PrimateExtension::D;
//  }
//  if (RequiredExtension == "Zvamo")
//    PrimateExtensions |= PrimateExtension::Zvamo;
//  if (RequiredExtension == "Zvlsseg")
//    PrimateExtensions |= PrimateExtension::Zvlsseg;
//
...
//}
//
//std::string PRVIntrinsic::getBuiltinTypeStr() const {
//  std::string S;
...
//}
//
//void PRVIntrinsic::emitCodeGenSwitchBody(raw_ostream &OS) const {
//  if (!getIRName().empty())
//    OS << "  ID = Intrinsic::primate_" + getIRName() + ";\n";
//  if (NF >= 2)
//    OS << "  NF = " + utostr(getNF()) + ";\n";
//  if (hasManualCodegen()) {
//    OS << ManualCodegen;
...
//}
//
//void PRVIntrinsic::emitIntrinsicMacro(raw_ostream &OS) const {
//  OS << "#define " << getName() << "(";
//  if (!InputTypes.empty()) {
//    ListSeparator LS;
...
//}
//
//void PRVIntrinsic::emitMangledFuncDef(raw_ostream &OS) const {
...
//
////===----------------------------------------------------------------------===//
//// PRVEmitter implementation
////===----------------------------------------------------------------------===//
//void PRVEmitter::createHeader(raw_ostream &OS) {
//
//  OS << "/*===---- primate_vector.h - Primate V-extension PRVIntrinsics "
//        "-------------------===\n"
//        " *\n"
//        " *\n"
...
//        " */\n\n";
//
//  OS << "#ifndef __PRIMATE_VECTOR_H\n";
//  OS << "#define __PRIMATE_VECTOR_H\n\n";
//
//  OS << "#include <stdint.h>\n";
//  OS << "#include <stddef.h>\n\n";
//
//  OS << "#ifndef __primate_vector\n";
...
//  OS << "#endif\n\n";
//
//  std::vector<std::unique_ptr<PRVIntrinsic>> Defs;
//  createPRVIntrinsics(Defs);
//
//  // Print header code
//  if (!HeaderCode.empty()) {
//    OS << HeaderCode;
...
//                   [](const std::unique_ptr<PRVIntrinsic> &A,
//                      const std::unique_ptr<PRVIntrinsic> &B) {
//                     return A->getPrimateExtensions() <
//                         B->getPrimateExtensions();
//                   });
//
//  // Print intrinsic functions with macro
//  emitArchMacroAndBody(Defs, OS, [](raw_ostream &OS, const PRVIntrinsic &Inst) {
//    Inst.emitIntrinsicMacro(OS);
//  });
//
//  OS << "#define __primate_v_intrinsic_overloading 1\n";
//
//  // Print Overloaded APIs
//  OS << "#define __prv_overloaded static inline "
...
//  OS << "\n#ifdef __cplusplus\n";
//  OS << "}\n";
//  OS << "#endif // __primate_vector\n";
//  OS << "#endif // __PRIMATE_VECTOR_H\n";
//}
//
//void PRVEmitter::createBuiltins(raw_ostream &OS) {
//  std::vector<std::unique_ptr<PRVIntrinsic>> Defs;
//  createPRVIntrinsics(Defs);
//
//  OS << "#if defined(TARGET_BUILTIN) && !defined(PRIMATEV_BUILTIN)\n";
//  OS << "#define PRIMATEV_BUILTIN(ID, TYPE, ATTRS) TARGET_BUILTIN(ID, TYPE, "
//        "ATTRS, \"experimental-v\")\n";
//  OS << "#endif\n";
//  for (auto &Def : Defs) {
//    OS << "PRIMATEV_BUILTIN(__builtin_prv_" << Def->getName() << ",\""
...
//      OS << "\"\")\n";
//  }
//  OS << "#undef PRIMATEV_BUILTIN\n";
//}
//
//void PRVEmitter::createCodeGen(raw_ostream &OS) {
//  std::vector<std::unique_ptr<PRVIntrinsic>> Defs;
//  createPRVIntrinsics(Defs);
...
//  // iteration.
//  PRVIntrinsic *PrevDef = Defs.begin()->get();
//  for (auto &Def : Defs) {
...
//    }
//    PrevDef = Def.get();
//    OS << "case Primate::BI__builtin_prv_" << Def->getName() << ":\n";
//  }
//  Defs.back()->emitCodeGenSwitchBody(OS);
//  OS << "\n";
//}
//
//void PRVEmitter::parsePrototypes(StringRef Prototypes,
//                                 std::function<void(StringRef)> Handler) {
...
//}
//
//std::string PRVEmitter::getSuffixStr(char Type, int Log2LMUL,
...
//}
//
//void PRVEmitter::createPRVIntrinsics(
//    std::vector<std::unique_ptr<PRVIntrinsic>> &Out) {
//  std::vector<Record *> RV = Records.getAllDerivedDefinitions("PRVBuiltin");
//  for (auto *R : RV) {
...
//        Optional<PRVTypes> Types = computeTypes(I, Log2LMUL, NF, ProtoSeq);
...
//        Out.push_back(std::make_unique<PRVIntrinsic>(
...
//
//Optional<PRVTypes>
//PRVEmitter::computeTypes(BasicType BT, int Log2LMUL, unsigned NF,
//                         ArrayRef<std::string> PrototypeSeq) {
...
//}
//
//Optional<PRVTypePtr> PRVEmitter::computeType(BasicType BT, int Log2LMUL,
//                                             StringRef Proto) {
...
//}
//
//void PRVEmitter::emitArchMacroAndBody(
//    std::vector<std::unique_ptr<PRVIntrinsic>> &Defs, raw_ostream &OS,
//    std::function<void(raw_ostream &, const PRVIntrinsic &)> PrintBody) {
//  uint8_t PrevExt = (*Defs.begin())->getPrimateExtensions();
//  bool NeedEndif = emitExtDefStr(PrevExt, OS);
//  for (auto &Def : Defs) {
//    uint8_t CurExt = Def->getPrimateExtensions();
//    if (CurExt != PrevExt) {
//      if (NeedEndif)
//        OS << "#endif\n\n";
...
//}
//
//bool PRVEmitter::emitExtDefStr(uint8_t Extents, raw_ostream &OS) {
//  if (Extents == PrimateExtension::Basic)
//    return false;
//  OS << "#if ";
//  ListSeparator LS(" && ");
//  if (Extents & PrimateExtension::F)
//    OS << LS << "defined(__primate_f)";
//  if (Extents & PrimateExtension::D)
//    OS << LS << "defined(__primate_d)";
//  if (Extents & PrimateExtension::Zfh)
//    OS << LS << "defined(__primate_zfh)";
//  if (Extents & PrimateExtension::Zvamo)
//    OS << LS << "defined(__primate_zvamo)";
//  if (Extents & PrimateExtension::Zvlsseg)
//    OS << LS << "defined(__primate_zvlsseg)";
//  OS << "\n";
//  return true;
//}
//
//namespace clang {
//void EmitPRVHeader(RecordKeeper &Records, raw_ostream &OS) {
//  PRVEmitter(Records).createHeader(OS);
//}
//
//void EmitPRVBuiltins(RecordKeeper &Records, raw_ostream &OS) {
//  PRVEmitter(Records).createBuiltins(OS);
//}
//
//void EmitPRVBuiltinCG(RecordKeeper &Records, raw_ostream &OS) {
