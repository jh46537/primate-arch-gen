//===--- Primate.cpp - Primate Helpers for Tools ----------------*- C++ -*-===//
//
...
struct PrimateExtensionVersion {
  StringRef Major;
  StringRef Minor;
};
...
// an underscore '_' e.g.: pr32i2_m2.
// Version number is divided into major and minor version numbers,
// separated by a 'p'. If the minor version is 0 then 'p0' can be
// omitted from the version string. E.g., pr32i2p0, pr32i2, pr32i2p1.
...
static bool getArchFeatures(const Driver &D, StringRef MArch,
  ...
  case 'e': {
    ...
    // Extension 'e' is not allowed in pr64.
    if (HasPR64)
  ...
  // Consume the base ISA version number and any '_' between prxxx and the
  // first extension
  ...
  // TODO: The 'q' extension requires pr64.
...
StringRef primate::getPrimateABI(const ArgList &Args,
  ...
  // 2. A default based on the architecture as determined by getPrimateArch
  ...
  // - On `primate{XLEN}-unknown-elf` we use the integer calling convention only.
...
StringRef primate::getPrimateArch(const llvm::opt::ArgList &Args,
  ...
  //
  // Clang does not yet support MULTILIB_REUSE, so we use `pr{XLEN}imafdc`
  // instead of `pr{XLEN}gc` though they are (currently) equivalent.
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_mcpu_EQ)) {
    StringRef MArch = llvm::Primate::getMArchFromMcpu(A->getValue());
  ...
  //
  // ilp32e -> pr32e
  // ilp32 | ilp32f | ilp32d -> pr32imafdc
  // lp64 | lp64f | lp64d -> pr64imafdc
  if (const Arg *A = Args.getLastArg(options::OPT_mabi_EQ)) {
  ...
  //
  // We deviate from GCC's defaults here:
  // - On `primate{XLEN}-unknown-elf` we default to `pr{XLEN}imac`
  // - On all other OSs we use `pr{XLEN}imafdc` (equivalent to `pr{XLEN}gc`)
  if (Triple.getArch() == llvm::Triple::primate32) {
    if (Triple.getOS() == llvm::Triple::UnknownOS)
      return "pr32imac";
    else
