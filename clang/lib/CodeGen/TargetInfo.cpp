...
//===----------------------------------------------------------------------===//
// Primate ABI Implementation
//===----------------------------------------------------------------------===//

namespace {
class PrimateABIInfo : public DefaultABIInfo {
private:
  // Size of the integer ('x') registers in bits.
  unsigned XLen;
  // Size of the floating point ('f') registers in bits. Note that the target
...
public:
  PrimateABIInfo(CodeGen::CodeGenTypes &CGT, unsigned XLen, unsigned FLen)
      : DefaultABIInfo(CGT), XLen(XLen), FLen(FLen) {}
...
void PrimateABIInfo::computeInfo(CGFunctionInfo &FI) const {
  QualType RetTy = FI.getReturnType();
  if (!getCXXABI().classifyReturnType(FI))
    FI.getReturnInfo() = classifyReturnType(RetTy);
...
// responsible for checking that if there is only a single field then that
// field is a float.
bool PrimateABIInfo::detectFPCCEligibleStructHelper(QualType Ty,
                                                    CharUnits CurOff,
                                                    llvm::Type *&Field1Ty,
                                                    CharUnits &Field1Off,
                                                    llvm::Type *&Field2Ty,
...
// value, fp+fp, or int+fp of appropriate size). If so, NeededArgFPRs and
// NeededArgGPRs are incremented appropriately.
bool PrimateABIInfo::detectFPCCEligibleStruct(QualType Ty,
                                              llvm::Type *&Field1Ty,
                                              CharUnits &Field1Off,
                                              llvm::Type *&Field2Ty,
                                              CharUnits &Field2Off,
...
// Field1Ty, Field1Off, Field2Ty, Field2Off. This method will create an
// appropriate coerceToType and unpaddedCoerceToType.
ABIArgInfo PrimateABIInfo::coerceAndExpandFPCCEligibleStruct(
    llvm::Type *Field1Ty, CharUnits Field1Off, llvm::Type *Field2Ty,
    CharUnits Field2Off) const {
  SmallVector<llvm::Type *, 3> CoerceElts;
  SmallVector<llvm::Type *, 2> UnpaddedCoerceElts;
...
ABIArgInfo PrimateABIInfo::classifyArgumentType(QualType Ty, bool IsFixed,
                                              int &ArgGPRsLeft,
                                              int &ArgFPRsLeft) const {
  assert(ArgGPRsLeft <= NumArgGPRs && "Arg GPR tracking underflow");
  Ty = useFirstFieldIfTransparentUnion(Ty);
...
ABIArgInfo PrimateABIInfo::classifyReturnType(QualType RetTy) const {
  if (RetTy->isVoidType())
    return ABIArgInfo::getIgnore();
...
Address PrimateABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
                                  QualType Ty) const {
  CharUnits SlotSize = CharUnits::fromQuantity(XLen / 8);
...
ABIArgInfo PrimateABIInfo::extendType(QualType Ty) const {
  int TySize = getContext().getTypeSize(Ty);
  // RV64 ABI requires unsigned 32 bit integers to be sign extended.
  if (XLen == 64 && Ty->isUnsignedIntegerOrEnumerationType() && TySize == 32)
    return ABIArgInfo::getSignExtend(Ty);
...
namespace {
class PrimateTargetCodeGenInfo : public TargetCodeGenInfo {
public:
  PrimateTargetCodeGenInfo(CodeGen::CodeGenTypes &CGT, unsigned XLen,
                           unsigned FLen)
      : TargetCodeGenInfo(std::make_unique<PrimateABIInfo>(CGT, XLen, FLen)) {}
  ...
  void setTargetAttributes(const Decl *D, llvm::GlobalValue *GV,
    ...
    const auto *Attr = FD->getAttr<PrimateInterruptAttr>();
    if (!Attr)
      return;

    const char *Kind;
    switch (Attr->getInterrupt()) {
    case PrimateInterruptAttr::user: Kind = "user"; break;
    case PrimateInterruptAttr::supervisor: Kind = "supervisor"; break;
    case PrimateInterruptAttr::machine: Kind = "machine"; break;
    }
...
const TargetCodeGenInfo &CodeGenModule::getTargetCodeGenInfo() {
  ...
  case llvm::Triple::primate64: {
    ...
    if (ABIStr.endswith("f"))
      ...
      ABIFLen = 64;
    return SetCGInfo(new PrimateTargetCodeGenInfo(Types, XLen, ABIFLen));
  }
