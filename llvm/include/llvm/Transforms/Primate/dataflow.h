...
public:
    ...
    void performBackwardAnalysis(Worklist &w) {
        ...
        for (succ_iterator SI = succ_begin(hotBlock), 
                            E = succ_end(hotBlock); SI != E; ++SI) {
            numSucc++;
        ...
        if ((*neighbourSpecificValues).find(hotBlock) != 
        ...
        if (numSucc == 0) setBoundaryCondition((*out)[hotBlock]); 
        ...
        if(changed) *(*in)[hotBlock] = *newIn;
            
        for (pred_iterator PI = pred_begin(hotBlock), 
                            E = pred_end(hotBlock); PI != E; ++PI) {
    ...
    void finalizeBackwardAnalysis(Function &func) {
        for (Function::iterator i = func.begin(), 
            ...
            if ((*neighbourSpecificValues).find(hotBlock) != 
                (*neighbourSpecificValues).end()) {
                for (succ_iterator SI = succ_begin(&*i), 
                                   SE = succ_end(&*i); SI != SE; SI++) {
...
protected:
    virtual void 
    setBoundaryCondition(FlowValueType*) = 0;
    
    virtual void 
    meetOp(FlowValueType* lhs, const FlowValueType* rhs) = 0;
    
    virtual FlowValueType* 
    initializeFlowValue(BasicBlock& b, SetType setType) = 0;
