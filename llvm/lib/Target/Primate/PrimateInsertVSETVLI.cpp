//===- PrimateInsertVSETVLI.cpp - Insert VSETVLI instructions ---------------===//
//
...
#define DEBUG_TYPE "primate-insert-vsetvli"
#define Primate_INSERT_VSETVLI_NAME "Primate Insert VSETVLI pass"

static cl::opt<bool> DisableInsertVSETVLPHIOpt(
    "primate-disable-insert-vsetvl-phi-opt", cl::init(false), cl::Hidden,
...
class VSETVLIInfo {
  ...
  // Fields from VTYPE.
  PrimateII::VLMUL VLMul = PrimateII::LMUL_1;
  uint8_t SEW = 0;
...
public:
  ...
  void setVTYPE(unsigned VType) {
    ...
    VLMul = PrimateVType::getVLMUL(VType);
    SEW = PrimateVType::getSEW(VType);
    TailAgnostic = PrimateVType::isTailAgnostic(VType);
    MaskAgnostic = PrimateVType::isMaskAgnostic(VType);
  }
  void setVTYPE(PrimateII::VLMUL L, unsigned S, bool TA, bool MA, bool MRO) {
    assert(isValid() && !isUnknown() &&
           "Can't set VTYPE for uninitialized or unknown");
    VLMul = L;
    SEW = S;
  ...
  unsigned encodeVTYPE() const {
    ...
    return PrimateVType::encodeVTYPE(VLMul, SEW, TailAgnostic, MaskAgnostic);
  }
  ...
  unsigned getSEWLMULRatio() const {
    ...
    unsigned LMul;
    bool Fractional;
    std::tie(LMul, Fractional) = PrimateVType::decodeVLMUL(VLMul);
  ...
  bool isCompatible(const VSETVLIInfo &InstrInfo) const {
    ...
    // it compatible.
    if (InstrInfo.hasAVLReg() && InstrInfo.AVLReg == Primate::NoRegister) {
...
class PrimateInsertVSETVLI : public MachineFunctionPass {
...
public:
  ...
  PrimateInsertVSETVLI() : MachineFunctionPass(ID) {
    initializePrimateInsertVSETVLIPass(*PassRegistry::getPassRegistry());
  }
  ...
  StringRef getPassName() const override { return Primate_INSERT_VSETVLI_NAME; }
...
char PrimateInsertVSETVLI::ID = 0;

INITIALIZE_PASS(PrimateInsertVSETVLI, DEBUG_TYPE, Primate_INSERT_VSETVLI_NAME,
                false, false)
...
static VSETVLIInfo computeInfoForInstr(const MachineInstr &MI, uint64_t TSFlags,
  ...
  PrimateII::VLMUL VLMul = PrimateII::getLMul(TSFlags);

  unsigned Log2SEW = MI.getOperand(NumOperands - 1).getImm();
  // A Log2SEW of 0 is an operation on mask registers only.
  bool MaskRegOp = Log2SEW == 0;
  unsigned SEW = Log2SEW ? 1 << Log2SEW : 8;
  assert(PrimateVType::isValidSEW(SEW) && "Unexpected SEW");
  ...
  bool ForceTailAgnostic = PrimateII::doesForceTailAgnostic(TSFlags);
  ...
  if (PrimateII::hasVLOp(TSFlags)) {
    ...
    InstrInfo.setAVLReg(Primate::NoRegister);
...
void PrimateInsertVSETVLI::insertVSETVLI(MachineBasicBlock &MBB, MachineInstr &MI,
  ...
  if (PrevInfo.isValid() && !PrevInfo.isUnknown() &&
    ...
    BuildMI(MBB, MI, DL, TII->get(Primate::PseudoVSETVLI))
        .addReg(Primate::X0, RegState::Define | RegState::Dead)
        .addReg(Primate::X0, RegState::Kill)
        .addImm(Info.encodeVTYPE())
        .addReg(Primate::VL, RegState::Implicit);
    return;
  }

  if (Info.hasAVLImm()) {
    BuildMI(MBB, MI, DL, TII->get(Primate::PseudoVSETIVLI))
        .addReg(Primate::X0, RegState::Define | RegState::Dead)
  ...
  Register DestReg = Primate::X0;
  if (AVLReg == Primate::X0)
    DestReg = MRI->createVirtualRegister(&Primate::GPRRegClass);
  BuildMI(MBB, MI, DL, TII->get(Primate::PseudoVSETVLI))
...
static VSETVLIInfo getInfoForVSETVLI(const MachineInstr &MI) {
  VSETVLIInfo NewInfo;
  if (MI.getOpcode() == Primate::PseudoVSETVLI) {
    Register AVLReg = MI.getOperand(1).getReg();
    assert((AVLReg != Primate::X0 || MI.getOperand(0).getReg() != Primate::X0) &&
           "Can't handle X0, X0 vsetvli yet");
    NewInfo.setAVLReg(AVLReg);
  } else {
    assert(MI.getOpcode() == Primate::PseudoVSETIVLI);
...
bool PrimateInsertVSETVLI::needVSETVLI(const VSETVLIInfo &Require,
  ...
  if (!CurInfo.isUnknown() && Require.hasAVLReg() &&
    ...
    if (MachineInstr *DefMI = MRI->getVRegDef(Require.getAVLReg())) {
      if (DefMI->getOpcode() == Primate::PseudoVSETVLI ||
          DefMI->getOpcode() == Primate::PseudoVSETIVLI) {
...
bool PrimateInsertVSETVLI::computeVLVTYPEChanges(const MachineBasicBlock &MBB) {
  ...
  for (const MachineInstr &MI : MBB) {
    ...
    if (MI.getOpcode() == Primate::PseudoVSETVLI ||
        MI.getOpcode() == Primate::PseudoVSETIVLI) {
      HadVectorOp = true;
    ...
    if (MI.isCall() || MI.isInlineAsm() || MI.modifiesRegister(Primate::VL) ||
        MI.modifiesRegister(Primate::VTYPE)) {
...
void PrimateInsertVSETVLI::computeIncomingVLVTYPE(const MachineBasicBlock &MBB) {
  BlockData &BBInfo = BlockInfo[MBB.getNumber()];
...
bool PrimateInsertVSETVLI::needVSETVLIPHI(const VSETVLIInfo &Require,
  ...
  if (!PHI || PHI->getOpcode() != Primate::PHI || PHI->getParent() != &MBB)
    return true;
  ...
  for (unsigned PHIOp = 1, NumOps = PHI->getNumOperands(); PHIOp != NumOps;
    ...
    if (!DefMI || (DefMI->getOpcode() != Primate::PseudoVSETVLI &&
                   DefMI->getOpcode() != Primate::PseudoVSETIVLI))
      return true;
...
void PrimateInsertVSETVLI::emitVSETVLIs(MachineBasicBlock &MBB) {
  VSETVLIInfo CurInfo;
  ...
  for (MachineInstr &MI : MBB) {
    ...
    if (MI.getOpcode() == Primate::PseudoVSETVLI ||
      ...
      assert(MI.getOperand(3).getReg() == Primate::VL &&
             MI.getOperand(4).getReg() == Primate::VTYPE &&
    ...
    if (PrimateII::hasSEWOp(TSFlags)) {
      ...
      if (PrimateII::hasVLOp(TSFlags)) {
        ...
        if (VLOp.isReg()) {
          ...
          VLOp.setReg(Primate::NoRegister);
          VLOp.setIsKill(false);
        }
        MI.addOperand(MachineOperand::CreateReg(Primate::VL, /*isDef*/ false,
                                                /*isImp*/ true));
      }
      MI.addOperand(MachineOperand::CreateReg(Primate::VTYPE, /*isDef*/ false,
                                              /*isImp*/ true));
...
bool PrimateInsertVSETVLI::runOnMachineFunction(MachineFunction &MF) {
...
FunctionPass *llvm::createPrimateInsertVSETVLIPass() {
  return new PrimateInsertVSETVLI();
}
