//===-- PrimateExpandAtomicPseudoInsts.cpp - Expand atomic pseudo instrs. ---===//
//
...
#define Primate_EXPAND_ATOMIC_PSEUDO_NAME                                        \
  "Primate atomic pseudo instruction expansion pass"

namespace {

class PrimateExpandAtomicPseudo : public MachineFunctionPass {
public:
  const PrimateInstrInfo *TII;
  static char ID;

  PrimateExpandAtomicPseudo() : MachineFunctionPass(ID) {
    initializePrimateExpandAtomicPseudoPass(*PassRegistry::getPassRegistry());
  }

  bool runOnMachineFunction(MachineFunction &MF) override;

  StringRef getPassName() const override {
    return Primate_EXPAND_ATOMIC_PSEUDO_NAME;
  }
...
char PrimateExpandAtomicPseudo::ID = 0;

bool PrimateExpandAtomicPseudo::runOnMachineFunction(MachineFunction &MF) {
...
bool PrimateExpandAtomicPseudo::expandMBB(MachineBasicBlock &MBB) {
  bool Modified = false;
...
bool PrimateExpandAtomicPseudo::expandMI(MachineBasicBlock &MBB,
                                       MachineBasicBlock::iterator MBBI,
                                       MachineBasicBlock::iterator &NextMBBI) {
...
static unsigned getLRForRMW32(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Acquire:
    return Primate::LR_W_AQ;
  case AtomicOrdering::Release:
    return Primate::LR_W;
  case AtomicOrdering::AcquireRelease:
    return Primate::LR_W_AQ;
  case AtomicOrdering::SequentiallyConsistent:
    return Primate::LR_W_AQ_RL;
  }
}
...
static unsigned getSCForRMW32(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Acquire:
    return Primate::SC_W;
  case AtomicOrdering::Release:
    return Primate::SC_W_RL;
  case AtomicOrdering::AcquireRelease:
    return Primate::SC_W_RL;
  case AtomicOrdering::SequentiallyConsistent:
    return Primate::SC_W_AQ_RL;
  }
}
...
static unsigned getLRForRMW64(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Monotonic:
    return Primate::LR_D;
  case AtomicOrdering::Acquire:
    return Primate::LR_D_AQ;
  case AtomicOrdering::Release:
    return Primate::LR_D;
  case AtomicOrdering::AcquireRelease:
    return Primate::LR_D_AQ;
  case AtomicOrdering::SequentiallyConsistent:
    return Primate::LR_D_AQ_RL;
  }
}
...
static unsigned getSCForRMW64(AtomicOrdering Ordering) {
  ...
  case AtomicOrdering::Acquire:
    return Primate::SC_D;
  case AtomicOrdering::Release:
    return Primate::SC_D_RL;
  case AtomicOrdering::AcquireRelease:
    return Primate::SC_D_RL;
  case AtomicOrdering::SequentiallyConsistent:
    return Primate::SC_D_AQ_RL;
  }
}
...
static void doAtomicBinOpExpansion(const PrimateInstrInfo *TII, MachineInstr &MI,
  ...
  case AtomicRMWInst::Nand:
    BuildMI(LoopMBB, DL, TII->get(Primate::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    BuildMI(LoopMBB, DL, TII->get(Primate::XORI), ScratchReg)
        .addReg(ScratchReg)
        .addImm(-1);
    break;
  }
...
static void insertMaskedMerge(const PrimateInstrInfo *TII, DebugLoc DL,
  ...
  BuildMI(MBB, DL, TII->get(Primate::XOR), ScratchReg)
      .addReg(OldValReg)
      .addReg(NewValReg);
  BuildMI(MBB, DL, TII->get(Primate::AND), ScratchReg)
      .addReg(ScratchReg)
      .addReg(MaskReg);
  BuildMI(MBB, DL, TII->get(Primate::XOR), DestReg)
      .addReg(OldValReg)
      .addReg(ScratchReg);
}

static void doMaskedAtomicBinOpExpansion(
    const PrimateInstrInfo *TII, MachineInstr &MI, DebugLoc DL,
  ...
  case AtomicRMWInst::Xchg:
    BuildMI(LoopMBB, DL, TII->get(Primate::ADDI), ScratchReg)
        .addReg(IncrReg)
        .addImm(0);
    break;
  case AtomicRMWInst::Add:
    BuildMI(LoopMBB, DL, TII->get(Primate::ADD), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Sub:
    BuildMI(LoopMBB, DL, TII->get(Primate::SUB), ScratchReg)
        .addReg(DestReg)
        .addReg(IncrReg);
    break;
  case AtomicRMWInst::Nand:
...
bool PrimateExpandAtomicPseudo::expandAtomicBinOp(
...
static void insertSext(const PrimateInstrInfo *TII, DebugLoc DL,
                       MachineBasicBlock *MBB, Register ValReg,
                       Register ShamtReg) {
  BuildMI(MBB, DL, TII->get(Primate::SLL), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
  BuildMI(MBB, DL, TII->get(Primate::SRA), ValReg)
      .addReg(ValReg)
      .addReg(ShamtReg);
}

bool PrimateExpandAtomicPseudo::expandAtomicMinMaxOp(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
    AtomicRMWInst::BinOp BinOp, bool IsMasked, int Width,
    MachineBasicBlock::iterator &NextMBBI) {
  ...
  BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW32(Ordering)), DestReg)
      .addReg(AddrReg);
  BuildMI(LoopHeadMBB, DL, TII->get(Primate::AND), Scratch2Reg)
      .addReg(DestReg)
      .addReg(MaskReg);
  BuildMI(LoopHeadMBB, DL, TII->get(Primate::ADDI), Scratch1Reg)
      .addReg(DestReg)
      .addImm(0);
  ...
  case AtomicRMWInst::Max: {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(Primate::BGE))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  case AtomicRMWInst::UMax:
    BuildMI(LoopHeadMBB, DL, TII->get(Primate::BGEU))
        .addReg(Scratch2Reg)
        .addReg(IncrReg)
        .addMBB(LoopTailMBB);
    break;
  ...
  BuildMI(LoopTailMBB, DL, TII->get(getSCForRMW32(Ordering)), Scratch1Reg)
      .addReg(AddrReg)
      .addReg(Scratch1Reg);
  BuildMI(LoopTailMBB, DL, TII->get(Primate::BNE))
      .addReg(Scratch1Reg)
      .addReg(Primate::X0)
      .addMBB(LoopHeadMBB);
...
bool PrimateExpandAtomicPseudo::expandAtomicCmpXchg(
  ...
  if (!IsMasked) {
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(Primate::BNE))
        .addReg(DestReg)
        .addReg(CmpValReg)
        .addMBB(DoneMBB);
    // .looptail:
    ...
    BuildMI(LoopHeadMBB, DL, TII->get(getLRForRMW(Ordering, Width)), DestReg)
        .addReg(AddrReg);
    BuildMI(LoopHeadMBB, DL, TII->get(Primate::AND), ScratchReg)
        .addReg(DestReg)
        .addReg(MaskReg);
    BuildMI(LoopHeadMBB, DL, TII->get(Primate::BNE))
        .addReg(ScratchReg)
...
INITIALIZE_PASS(PrimateExpandAtomicPseudo, "primate-expand-atomic-pseudo",
                Primate_EXPAND_ATOMIC_PSEUDO_NAME, false, false)

namespace llvm {

FunctionPass *createPrimateExpandAtomicPseudoPass() {
  return new PrimateExpandAtomicPseudo();
}
