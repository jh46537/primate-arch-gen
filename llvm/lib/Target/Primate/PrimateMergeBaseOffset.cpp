//===----- PrimateMergeBaseOffset.cpp - Optimise address calculations  ------===//
//
...
#define DEBUG_TYPE "primate-merge-base-offset"
#define Primate_MERGE_BASE_OFFSET_NAME "Primate Merge Base Offset"
namespace {

struct PrimateMergeBaseOffsetOpt : public MachineFunctionPass {
  static char ID;
  ...
  PrimateMergeBaseOffsetOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return Primate_MERGE_BASE_OFFSET_NAME;
  }
...
char PrimateMergeBaseOffsetOpt::ID = 0;
INITIALIZE_PASS(PrimateMergeBaseOffsetOpt, DEBUG_TYPE,
                Primate_MERGE_BASE_OFFSET_NAME, false, false)
...
bool PrimateMergeBaseOffsetOpt::detectLuiAddiGlobal(MachineInstr &HiLUI,
                                                  MachineInstr *&LoADDI) {
  if (HiLUI.getOpcode() != Primate::LUI ||
      HiLUI.getOperand(1).getTargetFlags() != PrimateII::MO_HI ||
  ...
  if (LoADDI->getOpcode() != Primate::ADDI ||
      LoADDI->getOperand(2).getTargetFlags() != PrimateII::MO_LO ||
...
// output from LoADDI.
void PrimateMergeBaseOffsetOpt::foldOffset(MachineInstr &HiLUI,
                                         MachineInstr &LoADDI,
...
bool PrimateMergeBaseOffsetOpt::matchLargeOffset(MachineInstr &TailAdd,
                                               Register GAReg,
                                               int64_t &Offset) {
  assert((TailAdd.getOpcode() == Primate::ADD) && "Expected ADD instruction!");
  ...
  if (OffsetTail.getOpcode() == Primate::ADDI) {
    ...
    if (AddiImmOp.getTargetFlags() != PrimateII::MO_None)
      return false;
    ...
    if (OffsetLui.getOpcode() != Primate::LUI ||
        LuiImmOp.getTargetFlags() != PrimateII::MO_None ||
    ...
    return true;
  } else if (OffsetTail.getOpcode() == Primate::LUI) {
...
bool PrimateMergeBaseOffsetOpt::detectAndFoldOffset(MachineInstr &HiLUI,
                                                  MachineInstr &LoADDI) {
...
bool PrimateMergeBaseOffsetOpt::runOnMachineFunction(MachineFunction &Fn) {
...
FunctionPass *llvm::createPrimateMergeBaseOffsetOptPass() {
  return new PrimateMergeBaseOffsetOpt();
}
