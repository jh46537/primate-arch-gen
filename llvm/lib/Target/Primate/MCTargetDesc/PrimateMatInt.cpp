//===- PrimateMatInt.cpp - Immediate materialisation -------------*- C++ -*--===//
//
...
#include "PrimateMatInt.h"
#include "MCTargetDesc/PrimateMCTargetDesc.h"
#include "llvm/ADT/APInt.h"
#include "llvm/Support/MathExtras.h"
using namespace llvm;

static int getInstSeqCost(PrimateMatInt::InstSeq &Res, bool HasPRC) {
  if (!HasPRC)
    return Res.size();
  ...
  for (auto Instr : Res) {
    ...
    // can take longer to execute than the single PRI instruction. Thus, we
    // consider that two PRC instruction are slightly more costly than one
    // PRI instruction. For longer sequences of PRC instructions the space
    // savings can be worth it, though. The costs below try to model that.
    if (!Compressed)
      Cost += 100; // Baseline cost of one PRI instruction: 100%.
    else
