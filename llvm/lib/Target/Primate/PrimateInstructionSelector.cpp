...
PrimateInstructionSelector::selectShiftMask(MachineOperand &Root) const {
  if (!Root.isReg())
    return std::nullopt;
  ...
  if (mi_match(ShAmtReg, MRI, m_GAdd(m_Reg(Reg), m_ICst(Imm)))) {
    ...
    if (Imm != 0 && Imm.urem(ShiftWidth) == 0) {
      ...
      ShAmtReg = MRI.createVirtualRegister(&Primate::GPRRegClass);
      unsigned NegOpc = Subtarget->is64Bit() ? Primate::SUBW : Primate::SUB;
      return {{[=](MachineInstrBuilder &MIB) {
        MachineIRBuilder(*MIB.getInstr())
            .buildInstr(NegOpc, {ShAmtReg}, {Register(Primate::X0), Reg});
        MIB.addReg(ShAmtReg);
      }}};
    }
    if (Imm.urem(ShiftWidth) == ShiftWidth - 1) {
      ...
      return {{[=](MachineInstrBuilder &MIB) {
        MachineIRBuilder(*MIB.getInstr())
            .buildInstr(Primate::XORI, {ShAmtReg}, {Reg})
            .addImm(-1);
        MIB.addReg(ShAmtReg);
      }}};
    }
...
InstructionSelector::ComplexRendererFns
PrimateInstructionSelector::selectSHXADDOp(MachineOperand &Root,
                                         unsigned ShAmt) const {
  using namespace llvm::MIPatternMatch;
  ...
  if (LeftShift.has_value()) {
    ...
    if (Mask.isShiftedMask()) {
      ...
      if (*LeftShift && Leading == 0 && C2.ult(Trailing) && Trailing == ShAmt) {
        Register DstReg = MRI.createVirtualRegister(&Primate::GPRRegClass);
        return {{[=](MachineInstrBuilder &MIB) {
          MachineIRBuilder(*MIB.getInstr())
              .buildInstr(Primate::SRLI, {DstReg}, {RegY})
  ...
  if (LeftShift.has_value() && Mask.isShiftedMask()) {
    ...
    if (Cond) {
      ...
      return {{[=](MachineInstrBuilder &MIB) {
        MachineIRBuilder(*MIB.getInstr())
            .buildInstr(Primate::SRLIW, {DstReg}, {RegY})
            .addImm(Trailing);
        MIB.addReg(DstReg);
      }}};
    }
...
InstructionSelector::ComplexRendererFns
PrimateInstructionSelector::selectSHXADD_UWOp(MachineOperand &Root,
                                            unsigned ShAmt) const {
  using namespace llvm::MIPatternMatch;
  MachineFunction &MF = *Root.getParent()->getParent()->getParent();
  MachineRegisterInfo &MRI = MF.getRegInfo();
  ...
  if (mi_match(
    ...
    if (Mask.isShiftedMask()) {
      ...
      if (Leading == 32 - ShAmt && C2 == Trailing && Trailing > ShAmt) {
        ...
        return {{[=](MachineInstrBuilder &MIB) {
          MachineIRBuilder(*MIB.getInstr())
              .buildInstr(Primate::SLLI, {DstReg}, {RegX})
...
InstructionSelector::ComplexRendererFns
PrimateInstructionSelector::selectAddrRegImm(MachineOperand &Root) const {
  MachineFunction &MF = *Root.getParent()->getParent()->getParent();
  MachineRegisterInfo &MRI = MF.getRegInfo();
...
/// Returns the PrimateCC::CondCode that corresponds to the CmpInst::Predicate CC.
/// CC Must be an ICMP Predicate.
static PrimateCC::CondCode getPrimateCCFromICmp(CmpInst::Predicate CC) {
  switch (CC) {
  default:
...
static void getOperandsForBranch(Register CondReg, MachineRegisterInfo &MRI,
                                 PrimateCC::CondCode &CC, Register &LHS,
                                 Register &RHS) {
  ...
  CC = getPrimateCCFromICmp(Pred);
  return;
}

bool PrimateInstructionSelector::select(MachineInstr &MI) {
  ...
  case TargetOpcode::G_FCONSTANT: {
    ...
    if (Size == 32 || (Size == 64 && Subtarget->is64Bit())) {
      Register GPRReg = MRI.createVirtualRegister(&Primate::GPRRegClass);
      if (!materializeImm(GPRReg, Imm.getSExtValue(), MIB))
        return false;

      unsigned Opcode = Size == 64 ? Primate::FMV_D_X : Primate::FMV_W_X;
      ...
      Register GPRRegHigh = MRI.createVirtualRegister(&Primate::GPRRegClass);
      Register GPRRegLow = MRI.createVirtualRegister(&Primate::GPRRegClass);
      ...
      MachineInstrBuilder PairF64 = MIB.buildInstr(
          Primate::BuildPairF64Pseudo, {DstReg}, {GPRRegLow, GPRRegHigh});
  ...
  case TargetOpcode::G_BRCOND: {
    Register LHS, RHS;
    PrimateCC::CondCode CC;
    getOperandsForBranch(MI.getOperand(0).getReg(), MRI, CC, LHS, RHS);

    auto Bcc = MIB.buildInstr(PrimateCC::getBrCond(CC), {}, {LHS, RHS})
  ...
  case TargetOpcode::G_BRJT: {
    ...
    auto SLL =
        MIB.buildInstr(Primate::SLLI, {&Primate::GPRRegClass}, {MI.getOperand(2)})
            .addImm(Log2_32(EntrySize));
    if (!SLL.constrainAllUses(TII, TRI, RBI))
      return false;

    // TODO: Use SHXADD. Moving to legalization would fix this automatically.
    auto ADD = MIB.buildInstr(Primate::ADD, {&Primate::GPRRegClass},
                              {MI.getOperand(0), SLL.getReg(0)});
    if (!ADD.constrainAllUses(TII, TRI, RBI))
      return false;

    unsigned LdOpc = EntrySize == 8 ? Primate::LD : Primate::LW;
    auto Dest =
        MIB.buildInstr(LdOpc, {&Primate::GPRRegClass}, {ADD.getReg(0)})
            .addImm(0)
    ...
    if (MJTI->getEntryKind() == MachineJumpTableInfo::EK_LabelDifference32) {
      Dest = MIB.buildInstr(Primate::ADD, {&Primate::GPRRegClass},
    ...
    auto Branch =
        MIB.buildInstr(Primate::PseudoBRIND, {}, {Dest.getReg(0)}).addImm(0);
  ...
  case TargetOpcode::G_BRINDIRECT:
    MI.setDesc(TII.get(Primate::PseudoBRIND));
  ...
  case TargetOpcode::G_FRAME_INDEX: {
    ...
    // ComplexPattern
    MI.setDesc(TII.get(Primate::ADDI));
    MI.addOperand(MachineOperand::CreateImm(0));
    return constrainSelectedInstRegOperands(MI, TII, TRI, RBI);
  }
...
bool PrimateInstructionSelector::selectMergeValues(
  ...
  if (!isRegInFprb(Dst, MRI) || !isRegInGprb(Lo, MRI) || !isRegInGprb(Hi, MRI))
    return false;
  MI.setDesc(TII.get(Primate::BuildPairF64Pseudo));
  return constrainSelectedInstRegOperands(MI, TII, TRI, RBI);
}

bool PrimateInstructionSelector::selectUnmergeValues(
    MachineInstr &MI, MachineIRBuilder &MIB, MachineRegisterInfo &MRI) const {
  ...
  if (!isRegInFprb(Src, MRI) || !isRegInGprb(Lo, MRI) || !isRegInGprb(Hi, MRI))
    return false;
  MI.setDesc(TII.get(Primate::SplitF64Pseudo));
  return constrainSelectedInstRegOperands(MI, TII, TRI, RBI);
}

bool PrimateInstructionSelector::replacePtrWithInt(MachineOperand &Op,
  ...
  MRI.setRegBank(PtrToInt.getReg(0), RBI.getRegBank(Primate::GPRRegBankID));
  Op.setReg(PtrToInt.getReg(0));
  return select(*PtrToInt);
}

void PrimateInstructionSelector::preISelLower(MachineInstr &MI,
                                            MachineIRBuilder &MIB,
                                            MachineRegisterInfo &MRI) {
  switch (MI.getOpcode()) {
...
void PrimateInstructionSelector::renderNegImm(MachineInstrBuilder &MIB,
...
void PrimateInstructionSelector::renderImmSubFromXLen(MachineInstrBuilder &MIB,
                                                    const MachineInstr &MI,
                                                    int OpIdx) const {
  assert(MI.getOpcode() == TargetOpcode::G_CONSTANT && OpIdx == -1 &&
         "Expected G_CONSTANT");
...
void PrimateInstructionSelector::renderImmSubFrom32(MachineInstrBuilder &MIB,
                                                  const MachineInstr &MI,
                                                  int OpIdx) const {
  assert(MI.getOpcode() == TargetOpcode::G_CONSTANT && OpIdx == -1 &&
         "Expected G_CONSTANT");
...
void PrimateInstructionSelector::renderImmPlus1(MachineInstrBuilder &MIB,
                                              const MachineInstr &MI,
                                              int OpIdx) const {
  assert(MI.getOpcode() == TargetOpcode::G_CONSTANT && OpIdx == -1 &&
         "Expected G_CONSTANT");
...
void PrimateInstructionSelector::renderImm(MachineInstrBuilder &MIB,
                                         const MachineInstr &MI,
                                         int OpIdx) const {
  assert(MI.getOpcode() == TargetOpcode::G_CONSTANT && OpIdx == -1 &&
         "Expected G_CONSTANT");
...
void PrimateInstructionSelector::renderTrailingZeros(MachineInstrBuilder &MIB,
                                                   const MachineInstr &MI,
                                                   int OpIdx) const {
  assert(MI.getOpcode() == TargetOpcode::G_CONSTANT && OpIdx == -1 &&
         "Expected G_CONSTANT");
...
const TargetRegisterClass *PrimateInstructionSelector::getRegClassForTypeOnBank(
    LLT Ty, const RegisterBank &RB) const {
  if (RB.getID() == Primate::GPRRegBankID) {
  ...
  if (RB.getID() == Primate::GPRRegBankID) {
    if (Ty.getSizeInBits() == 32)
      return &Primate::FPR32RegClass;
    if (Ty.getSizeInBits() == 64)
      return &Primate::FPR64RegClass;
  }
...
bool PrimateInstructionSelector::isRegInGprb(Register Reg,
                                           MachineRegisterInfo &MRI) const {
  return RBI.getRegBank(Reg, MRI, TRI)->getID() == Primate::GPRRegBankID;
}

bool PrimateInstructionSelector::isRegInFprb(Register Reg,
                                           MachineRegisterInfo &MRI) const {
  return RBI.getRegBank(Reg, MRI, TRI)->getID() == Primate::GPRRegBankID;
}

bool PrimateInstructionSelector::selectCopy(MachineInstr &MI,
                                          MachineRegisterInfo &MRI) const {
  Register DstReg = MI.getOperand(0).getReg();
  ...
  MI.setDesc(TII.get(Primate::COPY));
  return true;
}

bool PrimateInstructionSelector::selectImplicitDef(
    MachineInstr &MI, MachineIRBuilder &MIB, MachineRegisterInfo &MRI) const {
...
bool PrimateInstructionSelector::materializeImm(Register DstReg, int64_t Imm,
                                              MachineIRBuilder &MIB) const {
  MachineRegisterInfo &MRI = *MIB.getMRI();

  if (Imm == 0) {
    MIB.buildCopy(DstReg, Register(Primate::X0));
    RBI.constrainGenericRegister(DstReg, Primate::GPRRegClass, MRI);
    return true;
  }
  ...
  for (unsigned i = 0; i < NumInsts; i++) {
    Register TmpReg = i < NumInsts - 1
                          ? MRI.createVirtualRegister(&Primate::GPRRegClass)
                          : DstReg;
    const PrimateMatInt::Inst &I = Seq[i];
    MachineInstr *Result;
...
bool PrimateInstructionSelector::selectAddr(MachineInstr &MI,
                                          MachineIRBuilder &MIB,
                                          MachineRegisterInfo &MRI,
                                          bool IsLocal,
  ...
  if (TM.isPositionIndependent() || Subtarget->allowTaggedGlobals()) {
    if (IsLocal && !Subtarget->allowTaggedGlobals()) {
      ...
      // %pcrel_lo(auipc)).
      MI.setDesc(TII.get(Primate::PseudoLLA));
      return constrainSelectedInstRegOperands(MI, TII, TRI, RBI);
    }
    ...
    auto Result = MIB.buildInstr(Primate::PseudoLGA, {DefReg}, {})
                      .addDisp(DispMO, 0)
                      .addMemOperand(MemOp);
  ...
  case CodeModel::Small: {
    ...
    // (lui %hi(sym)) %lo(sym)).
    Register AddrHiDest = MRI.createVirtualRegister(&Primate::GPRRegClass);
    MachineInstr *AddrHi = MIB.buildInstr(Primate::LUI, {AddrHiDest}, {})
                               .addDisp(DispMO, 0, PrimateII::MO_HI);

    if (!constrainSelectedInstRegOperands(*AddrHi, TII, TRI, RBI))
      return false;

    auto Result = MIB.buildInstr(Primate::ADDI, {DefReg}, {AddrHiDest})
                      .addDisp(DispMO, 0, PrimateII::MO_LO);
...
bool PrimateInstructionSelector::selectSExtInreg(MachineInstr &MI,
                                               MachineIRBuilder &MIB) const {
  if (!STI.isPR64())
    return false;
  ...
  MachineInstr *NewMI =
      MIB.buildInstr(Primate::ADDIW, {Dst.getReg()}, {Src.getReg()}).addImm(0U);
...
bool PrimateInstructionSelector::selectSelect(MachineInstr &MI,
                                            MachineIRBuilder &MIB,
                                            MachineRegisterInfo &MRI) const {
  ...
  unsigned Opc = Primate::Select_GPR_Using_CC_GPR;
  if (RBI.getRegBank(DstReg, MRI, TRI)->getID() == Primate::GPRRegBankID) {
    unsigned Size = MRI.getType(DstReg).getSizeInBits();
    Opc = Size == 32 ? Primate::Select_FPR32_Using_CC_GPR
                     : Primate::Select_FPR64_Using_CC_GPR;
  }
...
static unsigned getFCmpOpcode(CmpInst::Predicate Pred, unsigned Size) {
  ...
  case CmpInst::FCMP_OLT:
    return Size == 32 ? Primate::FLT_S : Primate::FLT_D;
  case CmpInst::FCMP_OLE:
    return Size == 32 ? Primate::FLE_S : Primate::FLE_D;
  case CmpInst::FCMP_OEQ:
    return Size == 32 ? Primate::FEQ_S : Primate::FEQ_D;
  }
}
...
bool PrimateInstructionSelector::selectFPCompare(MachineInstr &MI,
                                               MachineIRBuilder &MIB,
                                               MachineRegisterInfo &MRI) const {
  ...
  if (legalizeFCmpPredicate(LHS, RHS, Pred, NeedInvert)) {
    if (NeedInvert)
      TmpReg = MRI.createVirtualRegister(&Primate::GPRRegClass);
    ...
    auto Cmp1 = MIB.buildInstr(getFCmpOpcode(CmpInst::FCMP_OEQ, Size),
                               {&Primate::GPRRegClass}, {LHS, LHS});
    if (!Cmp1.constrainAllUses(TII, TRI, RBI))
      return false;
    auto Cmp2 = MIB.buildInstr(getFCmpOpcode(CmpInst::FCMP_OEQ, Size),
                               {&Primate::GPRRegClass}, {RHS, RHS});
    if (!Cmp2.constrainAllUses(TII, TRI, RBI))
      return false;
    if (NeedInvert)
      TmpReg = MRI.createVirtualRegister(&Primate::GPRRegClass);
    auto And =
        MIB.buildInstr(Primate::AND, {TmpReg}, {Cmp1.getReg(0), Cmp2.getReg(0)});
  ...
  if (NeedInvert) {
    auto Xor = MIB.buildInstr(Primate::XORI, {DstReg}, {TmpReg}).addImm(1);
...
bool PrimateInstructionSelector::selectIntrinsicWithSideEffects(
    MachineInstr &MI, MachineIRBuilder &MIB, MachineRegisterInfo &MRI) const {
  ...
  default:
    return false;
  case Intrinsic::trap:
    MIB.buildInstr(Primate::UNIMP, {}, {});
    break;
  case Intrinsic::debugtrap:
    MIB.buildInstr(Primate::EBREAK, {}, {});
    break;
  }
...
void PrimateInstructionSelector::emitFence(AtomicOrdering FenceOrdering,
                                         SyncScope::ID FenceSSID,
                                         MachineIRBuilder &MIB) const {
  if (STI.hasStdExtZtso()) {
    ...
    if (FenceOrdering == AtomicOrdering::SequentiallyConsistent &&
      ...
      // fence rw, rw
      MIB.buildInstr(Primate::FENCE, {}, {})
          .addImm(PrimateFenceField::R | PrimateFenceField::W)
          .addImm(PrimateFenceField::R | PrimateFenceField::W);
      return;
    }
  ...
  case AtomicOrdering::AcquireRelease:
    ...
    MIB.buildInstr(Primate::FENCE_TSO, {}, {});
    return;
  case AtomicOrdering::Acquire:
    // fence acquire -> fence r, rw
    Pred = PrimateFenceField::R;
    Succ = PrimateFenceField::R | PrimateFenceField::W;
    break;
  case AtomicOrdering::Release:
    // fence release -> fence rw, w
    Pred = PrimateFenceField::R | PrimateFenceField::W;
    Succ = PrimateFenceField::W;
    break;
  ...
  MIB.buildInstr(Primate::FENCE, {}, {}).addImm(Pred).addImm(Succ);
}
