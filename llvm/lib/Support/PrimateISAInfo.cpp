//===-- PrimateISAInfo.cpp - RISC-V Arch String Parser ------------*- C++ -*-===//
//
...
struct PrimateSupportedExtension {
  const char *Name;
  /// Supported version.
  PrimateISAInfo::ExtensionVersion Version;

  bool operator<(const PrimateSupportedExtension &RHS) const {
...
static const char *PrimateGImplications[] = {
  "i", "m", "a", "f", "d", "zicsr", "zifencei"
};

// NOTE: This table should be sorted alphabetically by extension name.
static const PrimateSupportedExtension SupportedExtensions[] = {
    {"a", {2, 1}},
    {"c", {2, 0}},
    {"d", {2, 2}},
    {"e", {2, 0}},
...
// NOTE: This table should be sorted alphabetically by extension name.
static const PrimateSupportedExtension SupportedExperimentalExtensions[] = {
    {"zacas", {1, 0}},
...
void llvm::primateExtensionsHelp(StringMap<StringRef> DescMap) {

  outs() << "All available -march extensions for RISC-V\n\n";
  PrintExtension("Name", "Version", (DescMap.empty() ? "" : "Description"));

  PrimateISAInfo::OrderedExtensionMap ExtMap;
...
namespace {
struct LessExtName {
  bool operator()(const PrimateSupportedExtension &LHS, StringRef RHS) {
    return StringRef(LHS.Name) < RHS;
  }
  bool operator()(StringRef LHS, const PrimateSupportedExtension &RHS) {
    return LHS < StringRef(RHS.Name);
  }
};
} // namespace

static std::optional<PrimateISAInfo::ExtensionVersion>
...
void PrimateISAInfo::addExtension(StringRef ExtName,
                                PrimateISAInfo::ExtensionVersion Version) {
...
bool PrimateISAInfo::isSupportedExtensionFeature(StringRef Ext) {
  bool IsExperimental = stripExperimentalPrefix(Ext);

  ArrayRef<PrimateSupportedExtension> ExtInfo =
...
bool PrimateISAInfo::isSupportedExtension(StringRef Ext) {
  verifyTables();
...
bool PrimateISAInfo::isSupportedExtension(StringRef Ext, unsigned MajorVersion,
...
bool PrimateISAInfo::hasExtension(StringRef Ext) const {
...
bool PrimateISAInfo::compareExtension(const std::string &LHS,
...
std::vector<std::string> PrimateISAInfo::toFeatures(bool AddAllExtensions,
  ...
  if (AddAllExtensions) {
    for (const PrimateSupportedExtension &Ext : SupportedExtensions) {
      if (Exts.count(Ext.Name))
        continue;
      Features.push_back((llvm::Twine("-") + Ext.Name).str());
    }

    for (const PrimateSupportedExtension &Ext : SupportedExperimentalExtensions) {
      if (Exts.count(Ext.Name))
        continue;
...
static Error getExtensionVersion(StringRef Ext, StringRef In, unsigned &Major,
  ...
  if (PrimateISAInfo::isSupportedExtension(Ext, Major, Minor))
    return Error::success();
...
llvm::Expected<std::unique_ptr<PrimateISAInfo>>
PrimateISAInfo::parseFeatures(unsigned XLen,
                            const std::vector<std::string> &Features) {
  assert(XLen == 32 || XLen == 64);
  std::unique_ptr<PrimateISAInfo> ISAInfo(new PrimateISAInfo(XLen));
  ...
  return PrimateISAInfo::postProcessAndChecking(std::move(ISAInfo));
}

llvm::Expected<std::unique_ptr<PrimateISAInfo>>
PrimateISAInfo::parseNormalizedArchString(StringRef Arch) {
...
llvm::Expected<std::unique_ptr<PrimateISAInfo>>
PrimateISAInfo::parseArchString(StringRef Arch, bool EnableExperimentalExtension,
                              bool ExperimentalExtensionVersionCheck,
                              bool IgnoreUnknown) {
  // Primate ISA strings must be lowercase.
  if (llvm::any_of(Arch, isupper)) {
    return createStringError(errc::invalid_argument,
                             "string must be lowercase");
  }

  bool HasPR64 = Arch.starts_with("pr64");
  // ISA string must begin with pr32 or pr64.
  if (!(Arch.starts_with("pr32") || HasPR64) || (Arch.size() < 5)) {
    return createStringError(
        errc::invalid_argument,
        "string must begin with pr32{i,e,g} or pr64{i,e,g}");
  }

  unsigned XLen = HasPR64 ? 64 : 32;
  std::unique_ptr<PrimateISAInfo> ISAInfo(new PrimateISAInfo(XLen));
  ...
  case 'i':
    break;
  case 'g':
    // g expands to extensions in PrimateGImplications.
  ...
  if (Baseline == 'g') {
    ...
    // ISA spec.
    for (const auto *Ext : PrimateGImplications) {
...
Error PrimateISAInfo::checkDependency() {
...
void PrimateISAInfo::updateImplication() {
...
void PrimateISAInfo::updateCombination() {
...
void PrimateISAInfo::updateFLen() {
  FLen = 0;
...
void PrimateISAInfo::updateMinVLen() {
  for (auto const &Ext : Exts) {
...
void PrimateISAInfo::updateMaxELen() {
...
std::string PrimateISAInfo::toString() const {
  std::string Buffer;
...
llvm::Expected<std::unique_ptr<PrimateISAInfo>>
PrimateISAInfo::postProcessAndChecking(std::unique_ptr<PrimateISAInfo> &&ISAInfo) {
...
StringRef PrimateISAInfo::computeDefaultABI() const {
  if (XLen == 32) {
    if (hasExtension("e"))
      return "ilp32e";
    if (hasExtension("d"))
...
bool PrimateISAInfo::isSupportedExtensionWithVersion(StringRef Ext) {
  if (Ext.empty())
    return false;
...
std::string PrimateISAInfo::getTargetFeatureForExtension(StringRef Ext) {
  if (Ext.empty())
    return std::string();
